...
import {
  RiAddLine, RiEditLine, RiEyeLine, RiPercentLine, RiTimeLine,
  RiShieldFlashLine, RiLeafLine, RiCoinsLine, RiFlashlightLine,
  RiInformationLine, RiCalculatorLine, RiCalendarEventLine, RiCheckLine,
...
import { investmentPlans as initialPlans } from '../data/mockData';
...
export default function InvestmentPlans() {
  ...
  // Test Simulator in Drawer (String so user can completely backspace/clear)
  ...
  const openEdit = (plan) => {
    ...
    setFormData({
      name: plan.name,
      category: plan.category || 'Renewable Energy',
      customCategory: '',
      roi: plan.roi ? plan.roi.replace('%', '') : '18',
      durationDD: dd,
      durationMM: mm,
      durationYYYY: yyyy,
      minAmount: plan.minAmount ? plan.minAmount.replace(/[^0-9]/g, '') : '1000',
      maxAmount: plan.maxAmount ? plan.maxAmount.replace(/[^0-9]/g, '') : '50000',
      noMaxLimit: !plan.maxAmount,
      payoutInterval: plan.payoutInterval?.includes('Second') ? 'per_second' : 'daily',
  ...
  const handleSave = () => {
    ...
    const secRate = ((minVal * (roiVal / 100)) / (365 * 86400)).toFixed(6);
    const finalDuration = formatDurationString(formData.durationDD, formData.durationMM, formData.durationYYYY);
    ...
    if (editingPlan) {
      setPlans(plans.map(p => p.id === editingPlan.id ? {
        ...
        minAmount: `$${minVal.toLocaleString()}`,
        maxAmount: formData.noMaxLimit ? null : `$${(parseFloat(formData.maxAmount) || 50000).toLocaleString()}`,
        payoutInterval: formData.payoutInterval === 'per_second' ? 'Per Second (Live)' : 'Daily Payout',
  ...
  // Calculate live simulator values (handles empty string gracefully)
  const parsedTestAmount = parseFloat(testAmount) || 0;
  const roiNum = parseFloat(formData.roi) || 0;

  // Duration in Total Days for calculation
  const totalDays = (parseInt(formData.durationYYYY, 10) || 0) * 365 +
    ...
    <div className="space-y-6 animate-fade-in pb-8">
      ...
      <div className="card p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <SearchBar placeholder="Search plans by name or category..." value={search} onChange={setSearch} className="flex-1" />
          ...
          return (
            ...
            >
              ...
              <div className="flex gap-2 pt-3 border-t border-gray-100">
                <Button variant="secondary" size="sm" className="flex-1" icon={<RiEyeLine />} onClick={() => openEdit(plan)}>
                  Details
                </Button>
                <Button variant="primary" size="sm" className="flex-1" icon={<RiEditLine />} onClick={() => openEdit(plan)}>
                  Configure
                </Button>
              </div>
            </div>
      ...
      {filtered.length === 0 && (
        <div className="card p-12 text-center">
          <p className="text-gray-400">No investment plans found matching your search.</p>
        </div>
      )}
      ...
      <Modal
        ...
        title={editingPlan ? `Configure Plan: ${editingPlan.name}` : 'Create New Investment Plan'}
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-2">
              {[
                { id: 'Renewable Energy', label: 'Renewable Energy', icon: RiLeafLine, color: 'text-emerald-600' },
                { id: 'Precious Metal', label: 'Precious Metal', icon: RiCoinsLine, color: 'text-amber-600' },
                { id: 'custom', label: 'Custom', icon: RiAddLine, color: 'text-gold-600' },
              ].map(cat => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex items-center rounded-xl border border-gray-200 focus-within:border-gold-400 focus-within:ring-2 focus-within:ring-gold-100 bg-white overflow-hidden transition-all shadow-2xs">
                <span className="pl-3.5 pr-1 text-gray-500 font-bold text-sm select-none">$</span>
                <input
                  type="text"
                  inputMode="numeric"
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <label className="flex items-center gap-1 text-[11px] text-gray-500 cursor-pointer">
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, noMaxLimit: e.target.checked })}
                    className="rounded border-gray-300 text-gold-500 focus:ring-gold-400"
                  />
                  No Limit
                </label>
              </div>
              <div className={`flex items-center rounded-xl border border-gray-200 focus-within:border-gold-400 focus-within:ring-2 focus-within:ring-gold-100 overflow-hidden transition-all shadow-2xs ${
                ...
                <input
                  ...
                  className={`w-full py-2.5 pr-3 bg-transparent border-none outline-none font-semibold text-gray-800 text-sm ${
                    formData.noMaxLimit ? 'text-gray-400 cursor-not-allowed' : ''
                  }`}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center rounded-xl border border-gray-200 focus-within:border-gold-400 focus-within:ring-2 focus-within:ring-gold-100 bg-white overflow-hidden transition-all shadow-2xs">
                ...
                />
                <span className="pr-3.5 text-gray-400 font-bold text-sm select-none">%</span>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-[11px] font-bold text-gold-700 bg-gold-50 px-2 py-0.5 rounded-md border border-gold-200/60 shadow-2xs truncate max-w-[150px]">
                  {formatDurationString(formData.durationDD, formData.durationMM, formData.durationYYYY)}
                </span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="flex items-center rounded-xl border border-gray-200 focus-within:border-gold-400 focus-within:ring-2 focus-within:ring-gold-100 bg-white overflow-hidden transition-all shadow-2xs">
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, durationDD: e.target.value.replace(/[^0-9]/g, '') })}
                  />
                  <span className="pr-2 text-[10px] text-gray-400 font-bold uppercase select-none">D</span>
                </div>
                ...
                <div className="flex items-center rounded-xl border border-gray-200 focus-within:border-gold-400 focus-within:ring-2 focus-within:ring-gold-100 bg-white overflow-hidden transition-all shadow-2xs">
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, durationMM: e.target.value.replace(/[^0-9]/g, '') })}
                  />
                  <span className="pr-2 text-[10px] text-gray-400 font-bold uppercase select-none">M</span>
                </div>
                ...
                <div className="flex items-center rounded-xl border border-gray-200 focus-within:border-gold-400 focus-within:ring-2 focus-within:ring-gold-100 bg-white overflow-hidden transition-all shadow-2xs">
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, durationYYYY: e.target.value.replace(/[^0-9]/g, '') })}
                  />
                  <span className="pr-2 text-[10px] text-gray-400 font-bold uppercase select-none">Y</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className={`p-3 rounded-xl border cursor-pointer flex items-center gap-2.5 transition-all ${
                ...
                <input
                  ...
                  onChange={() => setFormData({ ...formData, payoutInterval: 'per_second' })}
                  className="text-gold-500 focus:ring-gold-400"
                />
                <div>
                  <p className="text-xs font-bold text-gray-800 flex items-center gap-1">
                    <RiFlashlightLine className="text-amber-500" /> Real-time Per Second
                  </p>
                  <p className="text-[10px] text-gray-400">Live per-second stream</p>
                </div>
              </label>
              ...
              <label className={`p-3 rounded-xl border cursor-pointer flex items-center gap-2.5 transition-all ${
                ...
                <input
                  ...
                  onChange={() => setFormData({ ...formData, payoutInterval: 'daily' })}
                  className="text-gold-500 focus:ring-gold-400"
                />
                <div>
                  <p className="text-xs font-bold text-gray-800 flex items-center gap-1">
                    <RiCalendarEventLine className="text-blue-500" /> Daily Payout
                  </p>
                  <p className="text-[10px] text-gray-400">Settled daily at 00:00 UTC</p>
                </div>
              </label>
            </div>
          </div>
          ...
          {formData.payoutInterval === 'per_second' ? (
            ...
            <div className="p-4 bg-gradient-to-br from-amber-50/80 via-gold-50/50 to-emerald-50/40 rounded-2xl border border-gold-200/70 space-y-3 animate-fade-in">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-gray-500">Live stream yield preview for user wallet</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 bg-white px-2.5 py-1.5 rounded-lg border border-gold-200 shadow-2xs">
                  <span className="text-xs font-bold text-gold-600 select-none">$</span>
                  <input
                    type="text"
                    inputMode="numeric"
                    value={testAmount}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-gold-200/40">
                <div className="p-2.5 bg-white rounded-xl text-center border border-gray-100 shadow-2xs">
                  <p className="text-[10px] text-gray-400 font-medium">Per Second</p>
              ...
              <div className="p-3 bg-white/90 rounded-xl border border-gold-100 text-[11px] text-gray-600 space-y-1">
                ...
                <p className="leading-relaxed">
                  Total annual yield is split across <strong>31,536,000 seconds</strong> (365d &times; 86,400s).
                  Formula: <code className="bg-gray-100 px-1 py-0.5 rounded font-mono text-[10px]">(${parsedTestAmount} &times; {formData.roi}%) &divide; 31,536,000 = ${perSecondYield.toFixed(6)}/sec</code>.
                </p>
              </div>
            </div>
          ) : (
            ...
            <div className="p-4 bg-gradient-to-br from-blue-50/70 via-indigo-50/40 to-gold-50/40 rounded-2xl border border-blue-200/70 space-y-3 animate-fade-in">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-gray-500">Scheduled 24-hour return & total maturity payout</p>
                  </div>
                </div>

                {/* Fully Clearable Test Amount Input */}
                <div className="flex items-center gap-1 bg-white px-2.5 py-1.5 rounded-lg border border-blue-200 shadow-2xs">
                  <span className="text-xs font-bold text-blue-600 select-none">$</span>
                  <input
                    type="text"
                    inputMode="numeric"
                    value={testAmount}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-blue-200/40">
                <div className="p-2.5 bg-white rounded-xl text-center border border-gray-100 shadow-2xs">
                  <p className="text-[10px] text-gray-400 font-medium">Daily Return (24h)</p>
                ...
                <div className="p-2.5 bg-white rounded-xl text-center border border-gray-100 shadow-2xs">
                  <p className="text-[10px] text-gray-400 font-medium">Weekly (7 Days)</p>
                  <p className="text-xs font-extrabold text-gray-800 font-mono mt-0.5">
                ...
                <div className="p-2.5 bg-white rounded-xl text-center border border-gray-100 shadow-2xs">
                  <p className="text-[10px] text-gray-400 font-medium">Monthly (30 Days)</p>
                  <p className="text-xs font-extrabold text-gray-800 font-mono mt-0.5">
                ...
                <div className="p-2.5 bg-white rounded-xl text-center border border-gray-100 shadow-2xs">
                  <p className="text-[10px] text-gray-400 font-medium">Total Term Profit</p>
              ...
              <div className="p-3 bg-white/90 rounded-xl border border-blue-100 flex items-center justify-between text-xs">
                <div>
                  <p className="font-bold text-gray-800">Total Return on Maturity ({totalDays} Days):</p>
                  <p className="text-[11px] text-gray-400">Principal (${parsedTestAmount.toLocaleString()}) + Total Profit (+${totalTermProfit.toFixed(2)})</p>
                </div>
